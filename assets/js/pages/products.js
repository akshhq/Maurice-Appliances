...
export function initProductsPage() {
  ...
  function filterAndRender() {
    ...
    list = list.filter((p) => {
      ...
      if (state.q) {
        const text =
          `${p.model} ${p.title} ${(p.specs || []).join(" ")} ${p.cat}`.toLowerCase();
      ...
      if (state.wattages.length > 0) {
        if (
          !p.wattage ||
          !state.wattages.some((w) => Math.abs(p.wattage - w) <= 100)
        )
          return false;
      }
      // Capacity
    ...
    list.sort((a, b) => {
      switch (state.sort) {
        ...
        case "warranty":
          return (b.warrantyYears || 1) - (a.warrantyYears || 1);
        default:
          return 0;
      }
    });
    ...
    grid.innerHTML = list
      .map((p) => renderProductCard(p, { basePath, showCompare: true }))
      .join("");
    ...
    if (state.cat) {
      ...
      if (catObj) {
        ...
        if (leadEl)
          leadEl.textContent =
            catObj.blurb ||
            "Engineered for Indian homes — BIS (ISI) certified for safety and lasting performance.";
  ...
  function renderActivePills() {
    ...
    if (state.cat) {
      ...
      pills.push({
        label: `Category: ${c ? c.name : state.cat}`,
        clear: () => {
          state.cat = "";
          syncCategoryChips();
        },
    ...
    state.bands.forEach((b) => {
      ...
      pills.push({
        label: labels[b] || b,
        clear: () => {
          state.bands = state.bands.filter((x) => x !== b);
          syncCheckboxes();
        },
      });
    });
    state.warrs.forEach((w) => {
      pills.push({
        label: `${w}+ Yr Warranty`,
        clear: () => {
          state.warrs = state.warrs.filter((x) => x !== w);
          syncCheckboxes();
        },
      });
    });
    ...
    document
      .getElementById("clearAllPillsBtn")
    ...
    cb.addEventListener("change", () => {
      ...
      if (type === "band") {
        state.bands = Array.from(
          document.querySelectorAll('input[data-filter="band"]:checked'),
        ).map((c) => c.value);
      } else if (type === "warranty") {
        state.warrs = Array.from(
          document.querySelectorAll('input[data-filter="warranty"]:checked'),
        ).map((c) => parseInt(c.value, 10));
      } else if (type === "wattage") {
        state.wattages = Array.from(
          document.querySelectorAll('input[data-filter="wattage"]:checked'),
        ).map((c) => parseInt(c.value, 10));
      }
      filterAndRender();
    });
