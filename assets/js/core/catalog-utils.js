...
export function getCategoryIconSvg(slug, stroke = 1.6) {
  const icons = {
    "room-heaters":
      '<path d="M7 3v6M12 3v6M17 3v6"/><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 15h8"/>',
    "water-heaters":
      '<rect x="6" y="3" width="12" height="14" rx="6"/><path d="M9 21h2M13 21h2"/><circle cx="12" cy="10" r="2"/>',
    ...
    "mixer-grinders":
      '<path d="M9 3h6l-1 5h-4z"/><rect x="8" y="8" width="8" height="8" rx="1"/><path d="M10 16v3h4v-3"/>',
    "gas-stoves":
      '<rect x="3" y="6" width="18" height="12" rx="2"/><circle cx="8" cy="12" r="2"/><circle cx="16" cy="12" r="2"/>',
    induction:
      '<rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/>',
    irons:
      '<path d="M4 15c0-4 4-7 9-7h7l-2 7z"/><path d="M4 15h13"/><circle cx="9" cy="11" r=".6"/>',
    chimneys: '<path d="M4 20V10l8-5 8 5v10"/><path d="M8 20v-6h8v6"/>',
    "kitchen-appliances":
      '<rect x="5" y="4" width="14" height="16" rx="2"/><path d="M9 8h6M9 12h6"/>',
    madhani:
      '<rect x="8" y="3" width="8" height="7" rx="1"/><path d="M12 10v9M9 19h6M10 13h4"/>',
    "coolers-ac":
      '<rect x="3" y="5" width="18" height="9" rx="2"/><path d="M7 18v1.4M11 18v2M15 18v1.4M19 18v2"/><path d="M6 9.5h.01M9.5 9.5h5"/>',
  };
...
export function getProductFrameSvg(cat = "room-heaters", extraClass = "") {
  ...
  const inner =
    frames[cat] ||
    `<circle cx="60" cy="60" r="34" fill="#fff" stroke="var(--ink)" stroke-width="2.5"/>`;
...
export function renderProductCard(p, options = {}) {
  ...
  if (
    cat === "water-heaters" ||
    (p.model && p.model.toLowerCase().includes("geyser"))
  ) {
    if (p.model.includes("1L") || p.model.includes("3L"))
      smartHighlight =
        '<span class="pcard__spec-pill pcard__spec-pill--highlight">1–2 Persons</span>';
    else if (
      p.model.includes("6L") ||
      p.model.includes("10L") ||
      p.model.includes("15L")
    )
      smartHighlight =
        '<span class="pcard__spec-pill pcard__spec-pill--highlight">2–4 Persons</span>';
    else if (
      p.model.includes("25L") ||
      p.model.includes("35L") ||
      p.model.includes("50L")
    )
      smartHighlight =
        '<span class="pcard__spec-pill pcard__spec-pill--highlight">4–6+ Persons</span>';
  } else if (p.wattage) {
    smartHighlight = `<span class="pcard__spec-pill pcard__spec-pill--highlight">${p.wattage}W</span>`;
  }

  const specPills = (p.specs || [])
    .slice(0, 3)
    .map((s) => `<span class="pcard__spec-pill">${s}</span>`)
    .join("");
...
function dispatchCompareUpdate() {
  document.dispatchEvent(
    new CustomEvent("maurice:compare-updated", {
