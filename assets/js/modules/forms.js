...
function initNewsletter() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showToast(
        "Subscribed successfully! You will receive new launch updates.",
        "success",
      );
      form.reset();
    } catch (error) {
...
function initAjaxForms() {
  document.querySelectorAll("[data-ajax-form]").forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      formData.append(
        "formType",
        form.dataset.formType || "general_submission",
      );
      ...
      try {
        ...
        setMsg(
          "Something went wrong. Please try again later or email customer.care@mauriceappliances.in",
          false,
        );
        showToast(
